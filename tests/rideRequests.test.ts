...
describe('POST /rides/:rideId/requests', () => {
  ...
  it('rejects a malformed ride id', async () => {
    const passenger = await registerUser('Ada Lovelace');

    const response = await requestToJoin('not-a-uuid', passenger);
...
describe('GET /rides/:rideId/requests', () => {
  ...
  it('rejects an unauthenticated request', async () => {
    ...
    const response = await request(app).get(`/api/rides/${ride.id}/requests`);
...
describe('PATCH /rides/:rideId/requests/:requestId/decline', () => {
  ...
  it('rejects an unauthenticated request', async () => {
    ...
    const response = await request(app).patch(
      `/api/rides/${ride.id}/requests/${created.body.data.id}/decline`,
    );
